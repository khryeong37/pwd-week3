...
function SubmissionsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      submissionAPI.listSubmissions(status === 'all' ? undefined : status),
  });
  ...
  const approveMutation = useMutation({
    mutationFn: async (submission) => {
      const payload = {
        ...
        recommendedMenu: Array.isArray(submission.recommendedMenu)
          ? submission.recommendedMenu
          : undefined,
      };
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id }) =>
      submissionAPI.updateSubmission(id, { status: 'rejected' }),
    onSuccess: () => {
        ...
        <tbody>
          {items.map((it) => (
            <tr key={it.id}>
              ...
              <td>
                <div>
                  <strong>{it.restaurantName}</strong> ({it.category})
                </div>
                <div>{it.location}</div>
                {it.priceRange && <div>가격대: {it.priceRange}</div>}
                {Array.isArray(it.recommendedMenu) &&
                  it.recommendedMenu.length > 0 && (
              ...
              <td>
                ...
                <Actions>
                  ...
                  <Button
                    onClick={() => rejectMutation.mutate({ id: it.id })}
